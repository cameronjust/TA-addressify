...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        try:
            ...
            username_to_associate_password_to = "addressify_api_key"
        
            # Load up previous passwords from store
            storedPassword, responseStatus = utils.loadPassword(self.splunkAppName, self.confFilename, self.getSessionKey(), "addressify_api_key")
            confInfo['addressify'].append('credential_key', "spur_api_key")
            confInfo['addressify'].append('credential', storedPassword)

            # Load up previous config - This file might not exist and will throw an error (might need to catch it)
            configContents = utils.configLoad(self.splunkAppName, self.confFilename, self.getSessionKey())
            confInfo['addressify'].append('useKVStore', configContents["useKVStore"])
            confInfo['addressify'].append('debugLogging', configContents["debugLogging"])
            confInfo['addressify'].append('proxy_settings', configContents["proxy_settings"])
...
#            self.appLogger.debug("%s,message=arguments passed to handleEdit %s." % (utils.fileFunctionLineNumber(),self.callerArgs.data))
            ...
            # Reload .conf so Splunk knows what you just setup. Doesn't seem to do anything so reloading whole app at the end of this function
#            utils.configReload(self.splunkAppName, self.confFilename, self.getSessionKey())

            # Write to app.conf to set app as configured : TODO: This should be done via the REST interfaces (not directly in file) so Splunk knows it happened otherwise the you'll get harrassed to setup the app until a restart.
