require([
    ...
    $.ajax({
        type: "GET",
        url: "../../../../en-US/splunkd/__raw/services/addressify/addressifysetup/addressify?output_mode=json",
        success: function(setup_parameters) {
            ...
            var ck_api_key = $("#addressify_api_key");
			...
			if (useKVStoreString == "1") {
				$("input[name='addressify_use_kvstore']").prop('checked', true);
			}
			
			if (debugLoggingString == "1") {
				$("input[name='addressify_debug_logging']").prop('checked', true);
			}
    ...
    $(submit_button).click(function(e) {
        ...
        credential_key_string = $("#addressify_api_key").val();
		proxy_settings_string = $("#proxy_settings").val();
		
		if ($("input[name='addressify_use_kvstore']").prop('checked')) {
			useKVStore = "1";
		} else {
			useKVStore = "0";
		}
		if ($("input[name='addressify_debug_logging']").prop('checked')) {
			debugLogging = "1";
		} else {
			debugLogging = "0";
		}
        ...
        $.ajax({
            type: "POST",
            url: "../../../../en-US/splunkd/__raw/services/addressify/addressifysetup/addressify",
            data: "credential_key=addressify_api_key&credential=" + credential_key_string + "&useKVStore=" + useKVStore + "&debugLogging=" + debugLogging + "&proxy_settings=" + proxy_settings_string ,
            success: function(text) {
                window.location.href = '../TA-addressify/search';
            },
            error: function() {
            }
        });
